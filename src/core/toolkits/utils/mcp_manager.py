...
class MCPManager:
    ...
    def shutdown(self):
        ...
        # fallback: force-terminate all spawned MCP server processes
        for process in self.processes:
            try:
                process.terminate()
            except ProcessLookupError:
                pass  # it's ok, the process may exit earlier
        # Give processes a moment to die, then force-kill
        time.sleep(1)
        for process in self.processes:
            try:
                process.kill()
        ...
        if self.loop_thread.is_alive():
            logger.warning("MCPManager loop thread did not stop within 10s timeout, proceeding anyway")
