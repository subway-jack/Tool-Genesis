...
try:
    ...
    class _DocstringStub:
        def __init__(self) -> None:
            self.params = []
            self.short_description = ""
            self.long_description = ""
    def parse(_text: str) -> "_DocstringStub":
        return _DocstringStub()
try:
    ...
    class SchemaError(Exception):
        pass
    class _JSONValidatorStub:
        @staticmethod
        def check_schema(_value):
            raise RuntimeError("jsonschema is required for schema validation.")
    JSONValidator = _JSONValidatorStub
from pydantic import BaseModel, create_model
from pydantic.fields import FieldInfo

from src.core.models import BaseModelBackend, ModelFactory
from src.core.types import ModelPlatformType, ModelType
from src.core.utils import get_pydantic_object_schema, to_pascal
