...
class WebSearchWithSummaryToolkit(BaseToolkit):
    """
    ...
    def __init__(
        ...
        *,
        ...
        # Truncation options
        max_words_before_truncate: Optional[int] = 10000,
        max_input_tokens_for_summarization: Optional[int] = 60000,
        summarizer_model_name: Optional[str] = "gpt-4o-mini",
    ) -> None:
        """
        Args:
            ...
            max_content_length: Maximum content length before triggering summarization (default: 5000).
            enable_summarization: Whether to enable LLM-based content summarization (default: True).
        ...
        # Content summarization settings
        self.max_content_length = max_content_length
        self.enable_summarization = enable_summarization
        # Truncation limits
        self.max_words_before_truncate = (
            max_words_before_truncate if max_words_before_truncate is not None else 10000
        )
        self.max_input_tokens_for_summarization = (
            max_input_tokens_for_summarization if max_input_tokens_for_summarization is not None else 60000
        )
        self.summarizer_model_name = summarizer_model_name or "gpt-4o-mini"
    ...
    def _open_with_exa(self, url: str) -> str:
        ...
        for item in result.get("results", []):
            ...
            summarized_text = self._summarize_content(text)
            formatted.append({"text": summarized_text})
        return json.dumps({"data": {"results": formatted}}, indent=2)
    ...
    def _open_with_direct_fetch(self, url: str) -> str:
        ...
        try:
            ...
            # Apply summarization if content is too long
            summarized_text = self._summarize_content(text)
            return json.dumps({"data": {"results": [{"text": summarized_text}]}}, indent=2)
        except Exception as e:
    ...
    def _truncate_by_tokens(self, text: str, max_tokens: int, model_name: str) -> str:
        try:
            import tiktoken
            try:
            ...
            # Safe fallback: approximate by characters
            return (text[: max(0, max_tokens * 3)]) + "\n\n[TRUNCATED]"

    def _summarize_content(self, content: str) -> str:
        """
        Use LLM to summarize lengthy content, removing redundant information while preserving key details.
        
        Args:
            content (str): The original content to summarize
            
        Returns:
            str: Summarized content or original content if summarization fails
        """
        # Always enforce a word-based hard cap first to protect logs and prompts
        if isinstance(content, str) and self.max_words_before_truncate:
            words = content.split()
            if len(words) > self.max_words_before_truncate:
                content = self._truncate_by_words(content, self.max_words_before_truncate)

        # If content is short or summarization disabled, return (after word truncation)
        if not self.enable_summarization or len(content) <= self.max_content_length:
            return content
            
        try:
            system_prompt = """
            You are a content summarization expert. Your task is to condense lengthy web content while preserving all important information, key facts, and essential details. 

Guidelines:
1. Remove redundant, repetitive, or unnecessary information
2. Preserve all factual data, numbers, dates, and specific details
3. Maintain the logical structure and flow of information
...
"""
            ...
            # Pre-truncate by tokens to fit model input window comfortably
            budget = max(self.max_input_tokens_for_summarization - 4096, 1024)
            trimmed = self._truncate_by_tokens(content, budget, self.summarizer_model_name)
            user_prompt = f"Please summarize the following content, keeping all important information while removing redundancy:\n\n{trimmed}"
