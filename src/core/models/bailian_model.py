...
class BailianModel(BaseModelBackend):
    r"""Bailian API in a unified BaseModelBackend interface.
    ...
    def __init__(
        ...
        api_key = api_key or os.environ.get("BAILIAN_API_KEY") or os.environ.get("OPENAI_API_KEY")
        url = url or os.environ.get("BAILIAN_API_BASE_URL") or os.environ.get("OPENAI_API_BASE_URL")
    ...
    def _request_chat_completion(
        ...
        request_config = self._prepare_request_config(request_config)
    ...
    def check_model_config(self):
        ...
        Raises:
            ValueError: If the model configuration dictionary contains any
                unexpected arguments to OpenAI API.
        """
        allowed_params = set(OPENAI_API_PARAMS) | {"extra_body", "enable_thinking"}
    ...
    def _prepare_request_config(self, request_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not isinstance(extra_body, dict):
            raise ValueError("`extra_body` must be a dict when provided.")

        if "enable_thinking" in extra_body:
            extra_body["enable_thinking"] = False if not stream else bool(extra_body["enable_thinking"])

        enable_thinking = request_config.pop("enable_thinking", None)
        if enable_thinking is not None:
            extra_body["enable_thinking"] = False if not stream else bool(enable_thinking)
        elif not stream:
